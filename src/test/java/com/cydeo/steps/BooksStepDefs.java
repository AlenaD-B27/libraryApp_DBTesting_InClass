...
public class BooksStepDefs {
    ...
    public void book_information_must_match_the_database_for(String bookName) {
      ...
      /*  String expectedBookName = DB_Util.getCellValue(1, "name");
        String expectedAuthorName = DB_Util.getCellValue(1, "author");
        String expectedISBN = DB_Util.getCellValue(1,"isbn");
        String expectedYear = DB_Util.getCellValue(1,"year");
        String expectedDesc = DB_Util.getCellValue(1,"description");
        */

        Map<String,String> bookInfo = DB_Util.getRowMap(1);

        Assert.assertEquals(bookInfo.get("name"), actualBookName);
        Assert.assertEquals(bookInfo.get("author"), actualAuthorName);
        Assert.assertEquals(bookInfo.get("isbn"), actualISBN);
        Assert.assertEquals(bookInfo.get("year"), actualYear);
        Assert.assertEquals(bookInfo.get("description"), actualDesc);
